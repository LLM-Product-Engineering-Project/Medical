...
Agent 기반 진료 관리 서비스 - 진료 녹음 텍스트를 분석하여 질병별 기록을 누적 관리하고 생활습관 알람을 생성합니다.

> 프로젝트 현황 및 기술 상세는 [CLAUDE.md](CLAUDE.md) 참조
...
`Recording_YYYYMMDD.txt` 형식의 진료 전사 텍스트 파일을 프로젝트 루트에 배치:
...
4. **Memory & Reflection** - 누적 진료 기록 기반 환자 상태 요약
5. **외부 검색** - Tavily를 통한 질병 관리 가이드라인 보완
6. **HITL 알람** - 환자 동의 시 생활습관 알람 및 일정표 생성
...
# 환자 ID 변경
run_many(input_dir, default_patient_id="patient_123")
...
| 패키지 설치 오류 | 가상환경 문제 | `rm -rf .venv && uv venv && uv pip install -e .` |
